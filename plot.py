...
y2_average = y2.rolling(window=50).mean()
y3_average = y3.rolling(window=50).mean()
y4_average = y4.rolling(window=50).mean()
y5_average = y5.rolling(window=50).mean()
...
plt.plot(x,y2_average, color = 'b', label = 'Shots Hit')
plt.plot(x,y3_average, color = 'r', label = "Avg Positional Diff")
plt.plot(x,y4_average, color = 'g', label = 'Avg Theta Diff')
plt.plot(x,y5_average, color = 'm', label = 'Shots Taken')
plt.xlabel('Episode')
